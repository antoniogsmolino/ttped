...
function videoEarnings(p, sold7, eps) {
  ...
  // Senza dati di saturazione si assume un mercato mediamente affollato (denominatore prudente).
  const denom = creators > 0 ? creators + 1 : 8;
  const salesPerVideo = sold7 / denom;
  const cappedSPV = Math.min(salesPerVideo, 60); // taglia outlier (capi a saturazione ~0)
...
// Score = impennata finestra (50%) + guadagno atteso €/video (35%) + €/vendita (15%), pesato per categoria.
