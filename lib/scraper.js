...
async function runScrape() {
  ...
  // Sharon = catalogo Italia (IT), Alena = catalogo UK (GB). Si scrappano entrambe le regioni.
  const regions = (process.env.SCRAPE_REGIONS || 'IT,GB').split(',').map((s) => s.trim());
  const seen = new Map();
  let lastError = null;
...
function computeRanking(view, sectionId, creator = 'sharon', limit = 20) {
  ...
  for (const p of Object.values(products)) {
    ...
    // Catalogo: Sharon = Italia (IT), Alena = UK (GB). Solo prodotti presenti in quel catalogo.
    const targetRegion = creator === 'alena' ? 'GB' : 'IT';
    if (!(p.regions || (p.region ? [p.region] : [])).includes(targetRegion)) continue;
