...
// Impennata % degli ultimi 7 giorni: media ultimi 3gg vs 4 precedenti + pendenza.
function spikeScore(p) {
  ...
  // Storico scarso: stima dall'aggregato 7gg vs 7gg precedenti (day14 - day7).
  if (p.sold14 && p.sold7) {
...
// Classifica: impennata vendite 7gg (55%) + commissione (30%) + volume (15%), pesata per categoria.
function computeTrends(limit = 20) {
  ...
  for (const p of Object.values(products)) {
    ...
    scored.push({
      ...
      trend: {
        ...
        growthPct: Math.round((p.incRate || spike) * 10) / 10,
        sold7,
  ...
  scored.sort((a, b) => b.trend.score - a.trend.score);
...
module.exports = { runScrape, computeTrends, getStatus, categoryMatch, DEFAULT_KEYWORDS };
