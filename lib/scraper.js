...
async function runScrape() {
  ...
  const catFilters = focusOn
    ...
    : [['', null]];
  // Solo saleRank (i best-seller) per non sovraccaricare: FastMoss rate-limita le raffiche di richieste.
      ...
      for (const dateType of [1, 2]) {
        for (let page = 1; page <= 4; page++) {
          try {
            ...
            if (process.env.DBG) console.error('DBG', catQ, 'dt' + dateType, 'p' + page, '→', j?.code, (j?.data?.rank_list || [])[0]?.region);
          ...
          await sleep(2500); // pause ampie: FastMoss free limita le raffiche (MAG_AUTH_3025)
        }
      }
    }
  }
