...
const DEFAULT_KEYWORDS = [
  // Abbigliamento donna
  'abito donna', 'vestito donna', 'pantaloni donna', 'jeans donna', 'shorts donna', 'bermuda donna',
  'gonna donna', 'top donna', 'maglia donna', 'camicia donna', 'felpa donna', 'giacca donna', 'completo donna', 'set donna',
  // Accessori donna
  'borsa donna', 'occhiali da sole donna', 'gioielli donna', 'cintura donna', 'cappello donna',
  // Intimo donna
  'reggiseno', 'intimo donna', 'pigiama donna', 'body modellante',
];
...
async function runScrape() {
  ...
  // Profilo Sharon: solo donna. l1_cid 2 = Womenswear & Underwear (abbigliamento + intimo), 8 = Accessori.
  const catFilters = focusOn
    ? [['&l1_cid=2', 'womenswear'], ['&l1_cid=8', null]]
    : [['', null]];
...
function computeRanking(view, sectionId, limit = 20) {
  const settings = getSettings();
  const focusOn = settings.categoryFocus?.enabled !== false;
  const products = readJSON(FILES.products, {});
  const date = today();
  ...
  for (const p of Object.values(products)) {
    ...
    const weight = focusOn ? 0.6 + 0.4 * cls.fit : 1;
    score *= weight;
...
// Le viste per una sezione.
function rankingsForSection(sectionId, limit = 20) {
  return {
    migliori: computeRanking('migliori', sectionId, limit),
    emergenti: computeRanking('emergenti', sectionId, limit),
  };
}

// Tutte le classifiche: per ogni sezione { migliori, emergenti }.
function computeAllRankings(limit = 20) {
  const out = {};
  for (const m of Models.MODELS) out[m.id] = rankingsForSection(m.id, limit);
  return out;
}

// Compat versione full locale (server.js, videogen): unione "migliori" di tutte le sezioni.
function computeTrends(limit = 20) {
  const all = computeAllRankings(limit);
  const merged = {};
  for (const m of Object.keys(all)) for (const p of all[m].migliori) merged[p.id] = p;
