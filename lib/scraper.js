...
async function runScrape() {
  ...
  const collect = (items, l1Hint) => {
    for (const p of items) {
      ...
      // La versione con più dati (trend 7gg, commissione) vince sul duplicato.
      if (!prev || (p.trendArr && !prev.trendArr)) seen.set(p.id, { ...prev, ...p });
      else {
        if (!prev.commissionRate && p.commissionRate) prev.commissionRate = p.commissionRate;
        if (!prev.l1Hint && p.l1Hint) prev.l1Hint = p.l1Hint;
      }
    }
  };
  ...
  //    Con focus moda donna attivo si interrogano direttamente le categorie TikTok Shop
  //    via l1_cid (2 = Womenswear & Underwear, 8 = Fashion Accessories).
  //    Piano free: max pagesize 10 e solo pagina 1; piani superiori paginano oltre.
  const catFilters = focusOn
    ? [['&l1_cid=2', 'womenswear'], ['&l1_cid=8', 'accessories'], ['', null]]
    : [['', null]];
  ...
  const matched = items.filter((p) => categoryMatch(p)).length;
...
function mergeHistory(items, date) {
  ...
  for (const p of items) {
    ...
    products[p.id] = { ...prev, ...fields, history, lastSeen: date };
  }
...
// Guadagno stimato per vendita in euro = prezzo × %commissione. È il numero che conta davvero.
function euroPerSale(p) {
  return Math.round(parsePrice(p.price) * (p.commissionRate || 0) / 100 * 100) / 100;
}

// Classifica per una finestra temporale.
// Score = impennata finestra (55%) + guadagno €/vendita (30%) + volume (15%), pesato per categoria.
// Ordinamento secondario: a parità di score, vince chi rende di più in euro a vendita.
function computeRanking(window, limit = 20) {
  const settings = getSettings();
  const focusOn = settings.categoryFocus?.enabled !== false;
  const products = readJSON(FILES.products, {});
  const date = today();
  ...
  for (const p of Object.values(products)) {
    ...
    const cat = categoryMatch(p);
    if (focusOn && !cat) continue;
    ...
    const volNorm = clamp(Math.log10(sold7 + 1) * 30, 0, 100);
    const weight = focusOn ? cat.weight : 1;
    const score = weight * (0.55 * spikeNorm + 0.30 * euroNorm + 0.15 * volNorm);
    ...
    scored.push({
      ...
      trend: {
        ...
        sold7,
        catLabel: cat ? cat.label : (p.cats?.[0] || ''),
        days: p.history.length,
        spark: p.history.slice(-14).map((h) => ({ date: h.date, v: h.soldDaily || 0 })),
      },
    });
  }
  // Ordinamento principale per score; a parità, vince il maggior guadagno €/vendita.
  scored.sort((a, b) => (b.trend.score - a.trend.score) || (b.trend.euroPerSale - a.trend.euroPerSale));
  return scored.slice(0, limit);
}

// Compat versione full (server.js, videogen): usa la classifica 7 giorni.
function computeTrends(limit = 20) {
  return computeRanking('d7', limit);
}

// Le tre classifiche per la dashboard: 7 giorni, 48 ore, 24 ore.
function computeAllRankings(limit = 20) {
  return {
    d7: computeRanking('d7', limit),
    h48: computeRanking('h48', limit),
    h24: computeRanking('h24', limit),
  };
}
...
module.exports = { runScrape, computeTrends, computeAllRankings, getStatus, categoryMatch, DEFAULT_KEYWORDS };
